    ...
    class Camera {
        ...
        public void Render(Game context, Location location) {
            if (location != _lastLocation || _changesSinceLastFrame) {
                ...
                Location maxChunk = Location.ToChunk(location + new Location(screenTileWidth, screenTileHeight, RenderingHeight));
                ...
                for (int xChunk = minChunk.X; xChunk <= maxChunk.X; xChunk++) {
                    for (int yChunk = minChunk.Y; yChunk <= maxChunk.Y; yChunk++) {
                        for (int zChunk = minChunk.Z; zChunk <= maxChunk.Z; zChunk++) {
                            Location currentChunk = new Location(xChunk, yChunk, zChunk);
                            ...
                            if (Cached(currentChunk)) {
                                ...
                                for (int x = 0; x < Chunk.Size; x++) {
                                    for (int y = 0; y < Chunk.Size; y++) {
                                        Location twoDimTileLoc = new Location(x, y, 0);
                                        ...
                                        for (int z = 0; z < RenderingHeight; z++) {
                                            Location currentLocation = new Location(x, y, z);
